...
import { isStickyEnabled, getStickyBinding, setStickyBinding, clearStickyBinding } from './sticky-session.js';
import { isExperimentalEnabled } from '../core/runtime-config.js';
import { readFileSync, writeFileSync, existsSync, unlinkSync, readdirSync } from 'fs';
import { config, log } from '../core/config.js';
import { safeAccountRef } from '../core/log-safety.js';
import { renameSyncWithRetry } from '../core/fs-atomic.js';
import { getEffectiveProxy } from '../core/proxy-config.js';
import { getTierModels, getModelKeysByEnum, MODELS, registerDiscoveredFreeModel } from '../models-catalog/models.js';
import { getLsAdmissionStatus, getLsMaintenanceRequests } from '../language-server/langserver.js';
...
async function fetchAndMergeModelCatalog() {
  ...
  try {
    const { getCascadeModelConfigs } = await import('../language-server/windsurf-api.js');
    const { mergeCloudModels } = await import('../models-catalog/models.js');
...
async function registerWithCodeium(idToken) {
  const { WindsurfClient } = await import('../language-server/client.js');
...
export function removeAccount(id) {
  ...
  import('../conversation-cache/conversation-pool.js').then(m => m.invalidateFor({ apiKey: account.apiKey })).catch(() => {});
...
export async function ensureLsForAccount(accountId) {
  const { ensureLs } = await import('../language-server/langserver.js');
...
export function reportBanSignal(apiKey, message, { windowMs = 30 * 60 * 1000 } = {}) {
  ...
  if (account._banSignalCount >= 2) {
    ...
    import('../conversation-cache/conversation-pool.js').then(m => m.invalidateFor({ apiKey })).catch(() => {});
    return true;
  }
  return false;
}
...
export async function refreshCredits(id) {
  ...
  try {
    const { getUserStatus } = await import('../language-server/windsurf-api.js');
...
export async function fetchUserStatus(id, { allowLsStart = true } = {}) {
  ...
  const { WindsurfClient } = await import('../language-server/client.js');
  const { ensureLs, getLsFor } = await import('../language-server/langserver.js');
  const proxy = getEffectiveProxy(account.id) || null;
  if (!allowLsStart) {
...
async function _probeAccountImpl(account, { allowLsStart = true, canary } = {}) {
  ...
  try {
    const { beginLsMaintenanceUse } = await import('../language-server/langserver.js');
  ...
  const { WindsurfClient } = await import('../language-server/client.js');
  const { getModelInfo } = await import('../models-catalog/models.js');
  const { ensureLs, getLsFor } = await import('../language-server/langserver.js');
  ...
  return { tier: account.tier, capabilities: account.capabilities };
    ...
    try {
      const { endLsMaintenanceUse } = await import('../language-server/langserver.js');
...
export async function initAuth() {
  ...
  const { ensureLs, shouldPrewarmDefaultLs } = await import('../language-server/langserver.js');
