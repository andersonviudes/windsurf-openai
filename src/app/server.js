...
import { handleChatCompletions } from '../chat-completions/chat.js';
import { handleMessages } from '../messages/messages.js';
import { handleResponses } from '../responses/responses.js';
import { handleModels } from '../models-catalog/models-handler.js';
...
async function route(req, res) {
  ...
  if (path === '/health') {
    ...
    if (qs.get('verbose') === '1' && validateApiKey(extractToken(req))) {
      try {
        const { poolStats } = await import('../conversation-cache/conversation-pool.js');
        const { cacheStats } = await import('../conversation-cache/cache.js');
        const { getLsStatus } = await import('../language-server/langserver.js');
        const { getSpecialAgentStatus } = await import('../special-agent/special-agent.js');
        const { getNativeBridgeStats } = await import('../native-bridge/native-bridge-stats.js');
        const { getNativeBridgeConfigStatus } = await import('../native-bridge/cascade-native-bridge.js');
