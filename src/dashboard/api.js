...
import {
  ...
  getDroughtSummary,
} from '../account-pool/auth.js';
import { restartLsForProxy } from '../language-server/langserver.js';
import { getLsStatus, stopLanguageServerAndWait, startLanguageServer, isLanguageServerRunning, getLsAdmissionStatus } from '../language-server/langserver.js';
import { getStats, resetStats, recordRequest } from '../core/stats.js';
import { cacheStats, cacheClear } from '../conversation-cache/cache.js';
import {
...
import { poolStats as convPoolStats, poolClear as convPoolClear } from '../conversation-cache/conversation-pool.js';
import { getLogs, subscribeToLogs, unsubscribeFromLogs } from '../core/logger.js';
import { getProxyConfig, getProxyConfigMasked, setGlobalProxy, setAccountProxy, removeProxy, getEffectiveProxy } from '../core/proxy-config.js';
import { MODELS, MODEL_TIER_ACCESS as _TIER_TABLE, getTierModels as _getTierModels } from '../models-catalog/models.js';
import { windsurfLogin, refreshFirebaseToken, reRegisterWithCodeium } from './windsurf-login.js';
import { getModelAccessConfig, setModelAccessMode, setModelAccessList, addModelToList, removeModelFromList, setDefaultModel } from '../core/model-access.js';
import { checkMessageRateLimit } from '../language-server/windsurf-api.js';
import { getNativeBridgeConfigStatus } from '../native-bridge/cascade-native-bridge.js';
import { getNativeBridgeStats } from '../native-bridge/native-bridge-stats.js';
import { assertPublicUrlHost } from '../attachments/image.js';
import { validateHostFormat } from '../core/net-safety.js';
...
} from '../core/quiet-window-updater.js';
...
export async function handleDashboardApi(method, subpath, body, req, res) {
      ...
      if (changed) {
        setTimeout(async () => {
          ...
          try {
            ...
            const m = await import('../language-server/langserver.js');
  ...
  if (subpath === '/langserver/update' && method === 'POST') {
    ...
    const { _poolKeys, restartLsForProxy: doRestart, getProxyByKey } =
      await import('../language-server/langserver.js');
    let restarted = 0;
    let restartErrors = [];
    try {
...
async function testProxy({ host, port, username, password, type }) {
  ...
  const { isSocks, createSocksTunnel } = await import('../core/socks.js');
