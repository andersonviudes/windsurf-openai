...
import { safeEmailRef, safeKeyRef, logHash } from '../core/log-safety.js';
import { isSocks, createSocksTunnel } from '../core/socks.js';
...
// 5xx retry helper: Windsurf's _devin-auth/* endpoints run on Vercel
// functions and intermittently return 504 / 503 (FUNCTION_INVOCATION_TIMEOUT).
// A transient failure of a single dispatch shouldn't surface to the user as
// "login failed", so retry 5xx with backoff (max 3 attempts).
// 4xx and 200 are returned directly without retry.
...
// On 2026-04-26 Windsurf changed the /_devin-auth/connections response from
//   { auth_method: { method: 'auth1', has_password: bool } }
// to
//   { connections: [{ id, type, enabled, client_id }, ...] }
// where type:'email' + enabled:true = the account supports email/password login.
// This function handles both the old and new shapes, returning a unified
// { method, hasPassword, raw }.
...
async function registerWithCodeium(token, fingerprint, proxy) {
  ...
  // this login flow.
  const { registerWithFirebaseToken } = await import('../language-server/windsurf-api.js');
...
export async function windsurfLogin(email, password, proxy = null) {
  ...
  if (lockMs != null) {
    ...
    const err = new Error(`Email ${email} is locally locked after ${EMAIL_LOCK_THRESHOLD} consecutive failed logins; try again in ${minutes} minute(s).`);
