...
import { extractImages } from '../attachments/image.js';
...
function resetCascadeTransportState(port) {
  // An HTTP/2 cancellation during Cascade warmup means the current LS session is unreliable; clear the reuse state so the next request rebuilds the session.
