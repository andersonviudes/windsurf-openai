...
function invalidateEntryForShutdown(entry) {
  ...
  import('../conversation-cache/conversation-pool.js')
  ...
  const promise = (async () => {
    ...
    proc.on('exit', (code, signal) => {
      ...
      if (gonePort) {
        ...
        import('../conversation-cache/conversation-pool.js').then(m => m.invalidateFor({ lsPort: gonePort, lsGeneration: goneGen })).catch(() => {});
      }
...
export async function restartLsForProxy(proxy) {
  ...
  if (entry?.port) {
    ...
    closeSessionForPort(entry.port);
    try {
      const m = await import('../conversation-cache/conversation-pool.js');
...
export function stopLanguageServer() {
  ...
  if (portsToClose.length) {
    import('../conversation-cache/conversation-pool.js').then(m => {
