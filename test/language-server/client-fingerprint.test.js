...
import { parseFields } from '../../src/language-server/proto.js';
...
describe('client fingerprint metadata', () => {
  it('uses the current Windsurf client version by default', async () => {
    ...
    try {
      const { buildMetadata } = await import('../../src/language-server/windsurf.js?fingerprint-default');
  ...
  it('uses WINDSURF_CLIENT_VERSION when set before import', async () => {
    ...
    try {
      const { buildMetadata } = await import('../../src/language-server/windsurf.js?fingerprint-env');
      const metadata = metadataStrings(buildMetadata('apikey', undefined, 'sess'));
  ...
  it('lets an explicit version override the default', async () => {
    const { buildMetadata } = await import('../../src/language-server/windsurf.js?fingerprint-explicit');
