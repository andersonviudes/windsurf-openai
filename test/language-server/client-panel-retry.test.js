...
import { getField, parseFields, writeMessageField, writeStringField, writeVarintField } from '../../src/language-server/proto.js';
...
async function countPendingReadUrlApprovals({ nativeAllowlist = ['read_url_content'] } = {}) {
  ...
  await withFakeLanguageServer((stream, headers) => {
    ...
    const { WindsurfClient } = await import('../../src/language-server/client.js');
