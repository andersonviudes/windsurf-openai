...
} from '../../src/language-server/langserver.js';

const __dirname = dirname(fileURLToPath(import.meta.url));
const AUTH_JS = readFileSync(join(__dirname, '..', '..', 'src/account-pool/auth.js'), 'utf8');
...
describe('language server resource policy', () => {
  ...
  test('default LS startup prewarm can be disabled for low-memory pools', () => {
    const src = readFileSync(join(__dirname, '..', '..', 'src/app/index.js'), 'utf8');
  ...
  test('LS admission is serialized and never evicts starting instances', () => {
    const ls = readFileSync(join(__dirname, '..', '..', 'src/language-server/langserver.js'), 'utf8');
  ...
  test('default prewarm is skipped when LS_MAX_INSTANCES leaves no proxy slot', () => {
    const ls = readFileSync(join(__dirname, '..', '..', 'src/language-server/langserver.js'), 'utf8');
    const index = readFileSync(join(__dirname, '..', '..', 'src/app/index.js'), 'utf8');
...
describe('process-level resilience (audit F)', () => {
  const INDEX_JS = readFileSync(join(__dirname, '..', '..', 'src/app/index.js'), 'utf8');
  ...
  test('SIGTERM shutdown awaits LS children exiting (no orphan-port race)', () => {
    ...
    assert.match(INDEX_JS, /import \{[^}]*stopLanguageServerAndWait[^}]*\} from '\.\.\/language-server\/langserver\.js'/);
  });
});
