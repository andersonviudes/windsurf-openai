...
import { isCascadeTransportError } from '../../src/language-server/client.js';
import { addAccountByKey, getApiKey, removeAccount } from '../../src/account-pool/auth.js';
import {
  chatStreamError,
...
} from '../../src/chat-completions/chat.js';
...
describe('stream error protocol', () => {
  ...
  it('routes oversized Connect frame parser errors to onError without throwing from data handlers', async () => {
    ...
    const grpc = await import(`../../src/language-server/grpc.js?connect-error-test=${Date.now()}`);
